...
uint32_t LoadStackFrame(int(*process)(int, char**), int argc, char** argv,
		uint32_t bottom, void(*cleaner)());
...
void write_fifo(int fd, char *buf, int n) {
	...
	if (n > (MAX_FIZE_SIZE - fifo_table[fd].curr_size)) {
		memcpy(fifo_table[fd].file, buf,
				MAX_FIZE_SIZE - fifo_table[fd].curr_size);
...
int kmain() {
	...
	logPID = CreateProcessAt("Login", (int(*)(int, char**)) logUser, 0, 0,
...
void block_process_in_kernel(int pid) {
	...
	if (aux->process->pid == pid) {
		...
		while (aux->next != NULL
				&& ((processNode*) aux->next)->process->pid != pid)
			aux = ((processNode*) aux->next);
		if (aux->next == NULL)
		{
			_yield();
...
void end_process(void) {
	...
	if (aux->process->pid == CurrentPID)
		...
		while (aux->next != NULL
				&& ((processNode *) aux->next)->process->pid != CurrentPID)
			aux = ((processNode *) aux->next);
		if (aux->next != NULL
			)
...
void clearTerminalBuffer_in_kernel(int ttyid) {
	terminals[ttyid].buffer.first_char = terminals[ttyid].buffer.actual_char
			+ 1 % BUFFER_SIZE;
...
void logUser(void) {
	...
	terminals[0].PID = CreateProcessAt("Shell0", (int(*)(int, char**)) shell, 0,
			0, (char**) 0, PAGE_SIZE, 2, 1);
	terminals[1].PID = CreateProcessAt("Shell1", (int(*)(int, char**)) shell, 1,
			0, (char**) 0, PAGE_SIZE, 2, 1);
	terminals[2].PID = CreateProcessAt("Shell2", (int(*)(int, char**)) shell, 2,
			0, (char**) 0, PAGE_SIZE, 2, 1);
	terminals[3].PID = CreateProcessAt("Shell3", (int(*)(int, char**)) shell, 3,
			0, (char**) 0, PAGE_SIZE, 2, 1);
	do_close(fd);
	_Sti();
	return;
...
void logout(int argc, char * argv[]) {
	...
	logPID = CreateProcessAt("logUsr", (int(*)(int, char**)) logUser,
			currentProcessTTY, 0, (char**) 0, PAGE_SIZE, 4, 1);
	_Sti();
}
